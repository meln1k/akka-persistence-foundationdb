...
final class FoundationDbSession(
  ...
  private implicit val ec = system.dispatchers.lookup("foundationdb-plugin-default-dispatcher")
  ...
  def runAsync[T](transaction: Transaction => Future[T]): Future[T] = underlying().flatMap { implicit db =>
    db.runAsync(tx => transaction(tx).toJava.toCompletableFuture).toScala
  }

  def readAsync[T](transaction: ReadTransaction => Future[T]): Future[T] = underlying().flatMap { implicit db =>
  ...
  def resolveDirectories(): Future[Directories] = async {
    ...
    val messagesDirectory = await(pluginDirectory.createOrOpen(db, List(messagesDirectoryName).asJava).toScala)
    val sequenceNrDirectory = await(pluginDirectory.createOrOpen(db, List(sequenceNrDirectoryName).asJava).toScala)
    val tagsDirectory = await(pluginDirectory.createOrOpen(db, List(tagsDirectoryName).asJava).toScala)
    val tagWatchesDirectory = await(pluginDirectory.createOrOpen(db, List(tagWatchesDirectoryName).asJava).toScala)
    val snapshotDirectory = await(pluginDirectory.createOrOpen(db, List(snapshotsDirectoryName).asJava).toScala)
    Directories(pluginDirectory, messagesDirectory, sequenceNrDirectory, tagsDirectory, tagWatchesDirectory, snapshotDirectory)
  }
