...
class ChunkedValueAssemblerSpec extends TestKit(ActorSystem("ChunkedByteStringReaderSpec")) with ImplicitSender
  ...
  "ChunkedByteStringReaderSpec" must {
    "assemble a single message" in {
      ...
      val result = Source(keyValues).via(ChunkedValueAssembler()).runWith(Sink.head).futureValue
    ...
    "assemble multiple messages" in {
      ...
      val chunkGroups = (1 to 10).map { groupNr =>
        (0 to 100).map { chunkNr =>
          Tuple.from(s"group-$groupNr", chunkNr: java.lang.Integer).pack() -> Array(chunkNr.toByte)
        }
      }
      ...
      val result = Source(keyValues).via(ChunkedValueAssembler()).runWith(Sink.seq).futureValue
      result shouldEqual chunkGroups.zipWithIndex.map { case (group, id)=>
        AssembledPayload(
    ...
    "assemble a message from a single chunk" in {
      val keyValue = new KeyValue(Tuple.from("message", 0: java.lang.Integer).pack(), ByteString("payload").toArray)
      val result = Source(List(keyValue)).via(ChunkedValueAssembler()).runWith(Sink.seq).futureValue
      result shouldEqual Seq(AssembledPayload(ByteString.fromArray(Tuple.from("message").pack()), ByteString("payload")))
    }

    "assemble multiple single chunk messages" in {

      val messages = (1 to 3).map(n => s"message$n" -> s"payload$n")
      val keyValues = messages.map { case (k,v) =>
        new KeyValue(Tuple.from(k, 0: java.lang.Integer).pack(), ByteString(v).toArray)
      }
      val assembledPayloads = messages.map { case (k, v) =>
    ...
    "drop the stream if an element is bigger than a limit" in {
      ...
      val result = Source(keyValues).via(ChunkedValueAssembler(9)).runWith(Sink.head).recover {
    ...
    "handle an empty source" in {
      val result = Source(List.empty[KeyValue]).via(ChunkedValueAssembler()).runWith(Sink.seq).futureValue
